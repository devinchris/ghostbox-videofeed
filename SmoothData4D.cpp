...
void SmoothQuaternionData::normalizeQuaternion(float q[4]) {
    ...
    // Skalarprodukt:
    // ARM optimiert (passiert auf der NPU)
    ...
    // Quadratwurzel -> Hardware-beschleunigt auf NPU
    float invMagnitude;
...
void SmoothQuaternionData::smoothQuaternions(Quaternion& _Quat, unsigned long currentTime) {
    ...
    // 1. Skalarprodukt (von Quaternionen) -> Mathematisch kompliziert ig -> Passiert auf unserer NPU (optimiert)
    float dotProduct;
