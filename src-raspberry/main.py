...
from calibration import MagnetometerCalibrator  # https://github.com/nliaudat/magnetometer_calibration/ 
...
class JetDevice:
    ...
    async def send_calibration_matrix(self, calibration_result):
        """Send magnetometer calibration parameters (soft + hard iron)."""
        if not self.is_connected():
            logger.warning(f"{self.name}: Cannot send calibration parameters - not connected")
            return False

        try:
            soft_matrix = np.asarray(
                calibration_result["soft_iron_matrix"], dtype=np.float32
            ).reshape(3, 3)
            hard_offset = np.asarray(
                calibration_result["hard_iron_offset"], dtype=np.float32
            ).reshape(3)
        except (KeyError, ValueError, TypeError) as exc:
            logger.error(f"{self.name}: Invalid calibration payload - {exc}")
            return False

        payload = np.concatenate([soft_matrix.flatten(), hard_offset])
        fmt = '<' + 'f' * payload.size

        async with self._write_lock:
            try:
                matrix_bytes = struct.pack(fmt, *payload.tolist())
                await self.client.write_gatt_char(JET_CHARACTERISTIC_COMMAND, matrix_bytes)
                await asyncio.sleep(0.2)
                logger.info("✓ Calibration parameters sent to Jet")
                return True
            except Exception:
...
# --------------------------------
# MARK: - Magnometer Calibration
# --------------------------------
def calculate_calibration_matrix(mag_data_buffer):
    """Calculate soft-iron matrix and hard-iron offset from raw magnetometer data."""
    sample_count = len(mag_data_buffer)
    logger.info(" Calculating magnetometer calibration from collected data...")
    logger.info(f"   Collected {sample_count} magnetometer samples")
    
    if sample_count == 0:
        raise ValueError("mag_data_buffer is empty;")
    if sample_count < MIN_CALIBRATION_SAMPLES:
        logger.warning(
            f"Nur {sample_count} samples... die Kalibrierung ist fehlerhaft"
        )

    try:
        mag_array = np.array(
            [[float(sample['x']), float(sample['y']), float(sample['z'])] for sample in mag_data_buffer],
            dtype=np.float64,
        )
    except (KeyError, TypeError, ValueError) as e:
        raise ValueError(f"Fehler bei der Kalibrierung: \n{e}") from e

    if mag_array.ndim != 2 or mag_array.shape[1] != 3:
        raise ValueError(
            f"mag_data_buffer in falscher Form: {mag_array.shape}"
        )

    # Remove NaNs/Infs that would break the ellipsoid fit
    finite_mask = np.isfinite(mag_array).all(axis=1)
    mag_array = mag_array[finite_mask]
    if len(mag_array) == 0:
        raise ValueError("Alle magnetometer samples waren invalid (NaN/Inf)")

    calibrator = MagnetometerCalibrator(
        magnetic_field_strength=EARTH_MAGNETIC_FIELD_UT
    )
    calibrator.calibrate(mag_array)

    soft_matrix = calibrator.A_1.astype(np.float32)
    hard_offset = calibrator.b.reshape(-1).astype(np.float32)

    logger.info("   Hard iron offset (µT): %s", np.array2string(hard_offset, precision=4))
    logger.info("   Soft iron matrix:\n%s", np.array2string(soft_matrix, precision=4))
    logger.info("✓✓✓ Kalibrierung durchgeführt. Yay. ✓✓✓")

    return {
        "soft_iron_matrix": soft_matrix,
        "hard_iron_offset": hard_offset,
    }
...
class SystemController:
    ...
    async def on_calibration_button_pressed(self):
        ...
        async with self._state_lock:
            ...
            calibration_result = calculate_calibration_matrix(self.jet.mag_data_buffer)
            
            # Send calibration matrix back to Jet
            logger.info("📤 Sending calibration parameters to Jet...")
            success = await self.jet.send_calibration_matrix(calibration_result)
